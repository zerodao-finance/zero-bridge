...
export const RequestTestingEnv = () => {
    ...
    function newTransferRequest() {
        //process
        txDispatch({ type: "ADD", payload: { type: "transfer", data: { msg: "test string", id: uuidv4(), type: "transfer" }}})
    }

    function completeTransferRequest(data) {
        txDispatch({type: "COMPLETE", payload: { type: "transfer", id: data.id, data: data}})
    }
    
    return (
        <>
            <button onClick={ () => { console.log(pending.transfer)}}>
                Print Transfers
            </button>
            <br />
            <button onClick={newTransferRequest}>
                Create Transfer
            </button>
            {
