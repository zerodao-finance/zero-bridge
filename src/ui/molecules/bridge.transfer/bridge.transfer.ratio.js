    ...
    return (
        <div className="flex flex-col gap-2 self-center text-black dark:text-white scale-[0.8] md:scale-[1]">
            <div className="flex flex-row w-full justify-between items-center px-5">
                <span className="flex items-center"><FaEthereum className="fill-gray-900 dark:fill-gray-300 h-[1.3rem] w-max text-xs pr-[0.5rem]"/><p className="text-xs">{`${ratio}%`}</p></span>
                <SliderInput ratio={ratio} effect={effect} />
                <span className="flex items-center font-bold"><p className="text-xs">{`${100-ratio}%`}</p><FaBtc className="fill-gray-900 dark:fill-gray-300 h-[1.3rem] w-max pl-[0.5rem]"/></span> 
            </div>
        </div>
    )
}