...
class BaseMonitor {
  ...
  attach(_observer) {
    ...
    if (this.observers.get(type).has(_observer))
  ...
  detach(_observer) {
    ...
    if (!this.observers.has(type))
      return console.log(`Subject: Observer type is invalid`);
    if (this.observers.get(type).has(_observer))
      this.observers.get(type).delete(_observer);
...
class TransactionMonitor {
  ...
  attach(observer) {
    ...
    if (!this.observers.has(type)) this.observers.set(type, new Set());
    if (this.observers.get(type).has(observer))
      return console.log(`\n Subject: Observer has been attached already`);
  ...
  async _checkIfLast(signer) {
    ...
    const isRequest = _.filter(requests, function (rq) {
      return rq.data.status == "pending";
    });
    ...
    if (isRequest.length != 0) {
      ...
      this.transferRequest = new TransferRequest(
        _.omit(isRequest[0].data, ["date", "status", "gatewayAddress", "dry"])
      );
  ...
  async _createTxn(to, value, ratio) {
    ...
    console.log(
      `\nSubject: ...data: \n { \n value: ${value} \n eth amount: ${
        ...
        process.env.CHAIN || process.env.REACT_APP_CHAIN
      } \n ratio: ${ratio} \n }`
    );
    ...
    const data = ethers.utils.defaultAbiCoder.encode(
      ...
      [
        ethers.utils.parseEther(
          parseFloat(String((Number(value) / 100) * ratio)).toFixed(8)
        ),
      ]
    );
  ...
  async _setProvider(provider) {
    if (!this.transferRequest || !provider)
      return new Error(
        `Subject: error setting provider no transferRequest or provider`
      );
  ...
  async _signTxn(signer) {
    if (!this.transferRequest || !signer)
      return new Error(`Subject: error signing no transferRequest or signer`);
    this._signature = await this.transferRequest.sign(signer);
    this._gatewayAddress = await this.transferRequest.toGatewayAddress();
    this._dry = await new TrivialUnderwriterTransferRequest(
      this.transferRequest
    ).dry(signer.provider, {
      from: "0x12fBc372dc2f433392CC6caB29CFBcD5082EF494",
    });
    this._key = await tools.storage.set({
      ...this.transferRequest,
      date: Date.now(),
      dry: this._dry,
    });
  ...
  async _transfer() {
    ...
    if (typeof this._dry == typeof Error)
      return console.log(`Subject: Loan will fail`);
  ...
  async _mockTransfer() {
    ...
    if (!this.transferRequest)
      return new Error(`Subject: error mocking transfer to RenVM`);
